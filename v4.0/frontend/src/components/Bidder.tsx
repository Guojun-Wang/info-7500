...
export function Bidder(): ReactElement {
  ...
  const [bidDutchAuctionContract, setBidDutchAuctionContract] = useState<Contract>();
  const [bidDutchAuctionContractAddr, setBidDutchAuctionContractAddr] = useState<string>('');

  const [reservePrice, setReservePrice] = useState<number>(DEFAULT_NUMBER_VAL);

  const [numBlocksAuctionOpen, setNumBlocksAuctionOpen] = useState<number>(DEFAULT_NUMBER_VAL);
  ...
  const [beginBlockNum, setBeginBlockNum] = useState<number>(DEFAULT_NUMBER_VAL);

  const [offerPriceDecrement, setOfferPriceDecrement] = useState<number>(DEFAULT_NUMBER_VAL);
  ...
  async function handleBid(
    ...
    event.preventDefault();
    if (bidPrice <= 0) {
      window.alert('Bid price must more than zero');
      return;
    }
    const options = {
  ...
  return (
    <>
      <StyledAuctionDiv>
        ...
        <StyledLabel htmlFor="bidPriceInput">Bid Price (wei)</StyledLabel>
        <StyledInput
          id="bidPriceInput"
          type="number"
          placeholder={
        ...
        <StyledLabel>Start Blocks Number:</StyledLabel>
        <div>
          {bidDutchAuctionContractAddr ? (
            beginBlockNum
          ) : (
        ...
        <StyledLabel>End Blocks Number:</StyledLabel>
        <div>
          {bidDutchAuctionContractAddr ? (
        ...
        <div>
          {bidDutchAuctionContractAddr ? (
            gotAcceptableBid ? (
              ...
              Math.max(
                reservePrice,
                reservePrice + (beginBlockNum + numBlocksAuctionOpen - curBlockNum) * offerPriceDecrement
              )
            )
          ) : (
            <em>{`<Contract not yet connected>`}</em>
        ...
        <StyledLabel>Accepted Bid Price:</StyledLabel>
        <div>
          {bidDutchAuctionContractAddr ? (
            gotAcceptableBid ? (
              acceptedBid
